...
"""
# MIT License
# Author: Tyler Barrus (barrust@gmail.com); KoishiMoe
...
from .constants import VERSION, URL
...
class MediaWiki(object):
    ...
    def __init__(self,
        ...
        """ DO NOT USE ME, USE MediaWiki.create() INSTEAD !!! """
    ...
    async def create(
        ...
        """ MediaWiki API Wrapper Instance
            ...
            Args:
                ...
                proxies (str): Proxy **URL** for aiohttp library to use.
                    It looks like 'http://your_proxy_url:your_proxy_port' or
                    'http://your_user:your_password@your_proxy_url:your_proxy_port' (If your proxy requires authentication)
        """
        ...
        if user_agent is not None:
            await self.set_user_agent(user_agent)
        await self.set_proxies(proxies)  # this will call self._reset_session()
    ...
    async def set_proxies(self, proxies):
        """ Turn on, off, or set proxy use through the aiohttp library """
    ...
    def refresh_interval(self):
        """ int: The interval at which the memoize cache is to be refreshed """
    ...
    async def login(self, username, password, strict=True):
        ...
        token_res = await self._get_response(params)
        ...
        res = await self._post_response(params)
    ...
    async def set_api_url(
        ...
        except (asyncio.TimeoutError, MediaWikiException):
    ...
    async def available_languages(self):
        ...
        if self.__available_languages is None:
            available = {}
            supported_languages = await self.supported_languages()
            for lang in supported_languages:
                try:
                    MediaWiki(lang=lang)
                    available[lang] = True
                except (aiohttp.ClientConnectionError, asyncio.TimeoutError, MediaWikiException,
                        MediaWikiAPIURLError):
    ...
    async def random(self, pages=1):
        ...
        request = await self.wiki_request(query_params)
    ...
    @memoize
    async def allpages(self, query="", results=10):
    ...
    @memoize
    async def search(self, query, results=10, suggestion=False):
        ...
        raw_results = await self.wiki_request(search_params)
    ...
    async def geosearch(
        ...
        if title is not None:
            if auto_suggest:
                title = await self.suggest(title)
            params["gspage"] = title
        else:
        ...
        raw_results = await self.wiki_request(params)
    ...
    async def opensearch(self, query, results=10, redirect=True):
        ...
        results = await self.wiki_request(query_params)
    ...
    @memoize
    async def prefixsearch(self, prefix, results=10):
        ...
        raw_results = await self.wiki_request(query_params)
    ...
    async def summary(self, title, sentences=0, chars=0, auto_suggest=True, redirect=True):
        ...
        return await page_info.summarize(sentences, chars)

    @memoize
    async def categorymembers(self, category, results=10, subcategories=True):
        ...
        while not finished:
            ...
            raw_res = await self.wiki_request(params)
    ...
    async def categorytree(self, category, depth=5):
    ...
    async def page(self, title=None, pageid=None, auto_suggest=True, redirect=True, preload=False,
        ...
        if title:
            if auto_suggest:
                temp_title = await self.suggest(title)
    ...
    async def wiki_request(self, params):
        ...
        req = await self._get_response(params)
    ...
    async def _get_site_info(self):
        ...
        response = await self.wiki_request({"meta": "siteinfo", "siprop": "extensions|general"})
    ...
    async def __cat_tree_rec(self, cat, depth, tree, level, categories, links):
        ...
        if cat not in categories:
            ...
            while True:
                ...
                try:
                    pag = await self.page("{0}:{1}".format(self.category_prefix, cat))
                    categories[cat] = pag
                    parent_cats = await categories[cat].categories()
                    links[cat] = await self.categorymembers(cat, results=None, subcategories=True)
                    break
                except PageError:
    ...
    async def _get_response(self, params):
        ...
        try:
            resp = await self._session.get(self._api_url, params=params, timeout=self._timeout, proxy=self._proxies)
            return await resp.json()
        except (JSONDecodeError, aiohttp.ContentTypeError):
            return {}

    async def _post_response(self, params):
        """ wrap a post call to the requests package """
        try:
            resp = await self._session.post(self._api_url, data=params, timeout=self._timeout, proxy=self._proxies)
            return await resp.json()
